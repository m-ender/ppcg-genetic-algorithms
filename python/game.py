...
def initialize_boards():
    ...
    for __ in xrange(INITIAL_SPECIMENS):
        board.add_specimen(Specimen(player, random.getrandbits(DNA_LENGTH)),
                           (random.randrange(0, BOARD_WIDTH), 0))
    for height in xrange(BOARD_HEIGHT):
        ...
        for width in xrange(BOARD_WIDTH):
            if random.random() < trap_frequency:
                board.add_trap(random.choice(trap.trap_types)(board), (width, height))
